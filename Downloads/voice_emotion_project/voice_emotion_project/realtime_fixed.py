"""
realtime_fixed.py - With audio boost
Run: python realtime_fixed.py
"""
...
DEVICE      = 19
TARGET_SR   = 22050
BOOST       = 50  # Amplify quiet mic input
...
def load_model():
    if not os.path.exists(MODEL_PATH):
        print("Model not found! Run: python train_fixed.py first")
        exit(1)
...
def display_results(emotion, confidence, scores, audio):
    ...
    fig.suptitle('Voice Emotion Recognition  |  Balanced SVM Model',
                 color='#00E5FF44', fontsize=10)

    axes[0].set_facecolor('#060D0A')
    t = np.linspace(0, DURATION, len(audio))
    axes[0].plot(t, audio * BOOST, color=color, linewidth=0.8)
    axes[0].fill_between(t, audio * BOOST, alpha=0.1, color=color)
...
def run():
    ...
    print("  Mic Boost: ON (50x amplification)")
    print("  Press ENTER to record | 'q' to quit")
    print("=" * 52)
    print()
    print("  Speak LOUDLY and close to mic!")
    print("  Angry    -- I HATE THIS! SO STUPID!")
    print("  Happy    -- HAHAHA YES! AMAZING!")
    print("  Sad      -- nobody loves me... so alone...")
    print("  Fearful  -- please dont hurt me...")
    print("  Neutral  -- the weather is nice today")
    print()
    ...
    while True:
        ...
        if user_input == 'q':
            ...
            break
        try:
            print(f"Recording {DURATION}s... SPEAK LOUDLY NOW!", end='', flush=True)
            ...
            # Check if audio captured
            if np.max(np.abs(audio)) < 0.0001:
                print("WARNING: Audio too quiet! Speak louder or move closer to mic!")
                continue

            print(f"Audio level: {np.max(np.abs(audio)):.4f} (boosted x{BOOST})")
            print("Analyzing...")
